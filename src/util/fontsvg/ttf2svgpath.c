...
{
	char cmd[1024];
	sprintf (cmd, "%s %s -l %d -h %d -o font.svg\n",
		BATIKTTF2SVG, fname, Min, Max);
	printf ("Invoking %s\n", cmd);
	system (cmd);
	
	FILE *fp = fopen("font.svg", "r");
...
void write_data (char *fname)
{
	...
	fprintf (fp, "static final Map<int, Metric> map;\n\n");
	fprintf (fp, "static {\n");
	fprintf (fp, "    map = new HashMap<int, Metric>();\n");
	fprintf (fp, "    GlyphMetric pairs = {\n");
	...
	for (int i=0; i<numC; i++)
	{
		...
		p = strstr (p, "d=\"") + 3;
		char *eol = strstr (p, "\" />");
		fprintf (fp, "    { 0x%x, new Metric(%d, %d, %d, %d, %d, \"",
			pp->wchar, pp->xMin,
			pp->xMax, pp->yMin,
			pp->yMax, pp->adv);
		while (p < eol) 
	...
	fprintf (fp, "};\n");
	fprintf (fp, "    for (GlyphMetrics pair : pairs) {\n");
	fprintf (fp, "        map.put(pair.cnum, pair.metric);\n");
	fprintf (fp, "    };\n");
